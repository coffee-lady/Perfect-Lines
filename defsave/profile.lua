...
local function obfuscate(input, key)
    if M.enable_obfuscation == false then
        return input
    end
...
function M.set_active_profile(profile)
    ...
    if M.verbose == true then
        log("Profile: New active profile " .. tostring(profile))
    end
...
function M.create_profile(profile, template_data, extra_data)
    if M.verbose == true then
        log("Profile: create_profile - " .. tostring(profile))
    end
...
function M.reset_profile(profile, template_data)
    if M.verbose == true then
        log("Profile: Resetting profile " .. tostring(profile))
    end
    M.profiles[profile] = {}
