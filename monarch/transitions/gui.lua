...
local function create()
    ...
    local transitions = {
        [monarch.TRANSITION.SHOW_IN] = {
            urls = {},
            transitions = {},
            in_progress_count = 0,
        },
        [monarch.TRANSITION.SHOW_OUT] = {
            urls = {},
            transitions = {},
            in_progress_count = 0,
        },
        [monarch.TRANSITION.BACK_IN] = {
            urls = {},
            transitions = {},
            in_progress_count = 0,
        },
        [monarch.TRANSITION.BACK_OUT] = {
            urls = {},
            transitions = {},
            in_progress_count = 0,
        },
        ...
        if t.in_progress_count == 0 then
            ...
            if #t.transitions > 0 then
                for i = 1, #t.transitions do
                    ...
                    transition.fn(transition.node, transition.node_data, transition.easing, transition.duration, transition.delay or 0,
                        function()
                            t.in_progress_count = t.in_progress_count - 1
    ...
    function instance.finish_transition(message_id, sender)
        local t = transitions[message_id]
        table.insert(t.urls, sender)
    ...
    function instance.handle(message_id, message, sender)
        ...
        elseif message_id == monarch.TRANSITION.SHOW_IN or message_id == monarch.TRANSITION.SHOW_OUT or message_id ==
            monarch.TRANSITION.BACK_IN or message_id == monarch.TRANSITION.BACK_OUT then
...
function M.in_right_out_left(node, duration, delay, easing)
    ...
    return M.create(node).show_in(M.slide_in_right, easing.OUT, duration, delay or 0).show_out(M.slide_out_left, easing.IN, duration,
               delay or 0).back_in(M.slide_in_left, easing.OUT, duration, delay or 0).back_out(M.slide_out_right, easing.IN, duration,
               delay or 0)
end
...
function M.in_left_out_right(node, duration, delay, easing)
    ...
    assert(duration, "You must provide a duration")
    easing = easing or easings.QUAD()
    return M.create(node).show_in(M.slide_in_left, easing.OUT, duration, delay or 0).show_out(M.slide_out_right, easing.IN, duration,
               delay or 0).back_in(M.slide_in_right, easing.OUT, duration, delay or 0).back_out(M.slide_out_left, easing.IN, duration,
               delay or 0)
end
...
function M.in_right_out_right(node, duration, delay, easing)
    ...
    assert(duration, "You must provide a duration")
    easing = easing or easings.QUAD()
    return M.create(node).show_in(M.slide_in_right, easing.OUT, duration, delay or 0).show_out(M.slide_out_right, easing.IN, duration,
               delay or 0).back_in(M.slide_in_right, easing.OUT, duration, delay or 0).back_out(M.slide_out_right, easing.IN, duration,
               delay or 0)
end
...
function M.in_left_out_left(node, duration, delay, easing)
    ...
    assert(duration, "You must provide a duration")
    easing = easing or easings.QUAD()
    return M.create(node).show_in(M.slide_in_left, easing.OUT, duration, delay or 0).show_out(M.slide_out_left, easing.IN, duration,
               delay or 0).back_in(M.slide_in_left, easing.OUT, duration, delay or 0).back_out(M.slide_out_left, easing.IN, duration,
               delay or 0)
end
...
function M.fade_in_out(node, duration, delay, easing)
    ...
    assert(duration, "You must provide a duration")
    easing = easing or easings.QUAD()
    return M.create(node).show_in(M.fade_in, easing.OUT, duration, delay or 0).show_out(M.fade_out, easing.IN, duration, delay or 0)
               .back_in(M.fade_in, easing.OUT, duration, delay or 0).back_out(M.fade_out, easing.IN, duration, delay or 0)
end
