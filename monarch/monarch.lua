...
local function focus_gained(screen, previous_screen)
    ...
    if screen.focus_url then
        msg.post(screen.focus_url, M.FOCUS.GAINED, {
            id = previous_screen and previous_screen.id,
        })
    else
...
local function focus_lost(screen, next_screen)
    ...
    if screen.focus_url then
        msg.post(screen.focus_url, M.FOCUS.LOST, {
            id = next_screen and next_screen.id,
        })
...
local function change_timestep(screen)
    if screen.proxy then
        ...
        msg.post(screen.proxy, "set_time_step", {
            mode = 0,
            factor = screen.timestep_below_popup,
        })
    end
end
...
local function show_out(screen, next_screen, cb)
    ...
    run_coroutine(screen, cb, function()
        ...
        notify_transition_listeners(M.SCREEN_TRANSITION_OUT_STARTED, {
            screen = screen.id,
            next_screen = next_screen.id,
        })
        change_context(screen)
        release_input(screen, next_screen)
        focus_lost(screen, next_screen)
        ...
        active_transition_count = active_transition_count - 1
        notify_transition_listeners(M.SCREEN_TRANSITION_OUT_FINISHED, {
            screen = screen.id,
            next_screen = next_screen.id,
        })
    end)
...
local function show_in(screen, previous_screen, reload, add_to_stack, cb)
    ...
    run_coroutine(screen, cb, function()
        active_transition_count = active_transition_count + 1
        notify_transition_listeners(M.SCREEN_TRANSITION_IN_STARTED, {
            screen = screen.id,
            previous_screen = previous_screen and previous_screen.id,
        })
        change_context(screen)
        ...
        if not ok then
            log("show_in()", err)
            active_transition_count = active_transition_count - 1
            notify_transition_listeners(M.SCREEN_TRANSITION_FAILED, {
                screen = screen.id,
            })
            return
        end
        ...
        active_transition_count = active_transition_count - 1
        notify_transition_listeners(M.SCREEN_TRANSITION_IN_FINISHED, {
            screen = screen.id,
            previous_screen = previous_screen and previous_screen.id,
        })
    end)
...
local function show_in_already_visible(screen, previous_screen, reload, add_to_stack, cb)
    log("show_in_already_visible()", screen.id)
    run_coroutine(screen, cb, function()
        change_context(screen)
        if add_to_stack then
            stack[#stack + 1] = screen
    ...
    run_coroutine(screen, cb, function()
        ...
        if previous_screen and not previous_screen.popup then
            transition(screen, M.TRANSITION.BACK_IN, {
                previous_screen = previous_screen.id,
                screen = screen.id,
            })
        end
        acquire_input(screen)
...
local function back_out(screen, next_screen, cb)
    ...
    run_coroutine(screen, cb, function()
        notify_transition_listeners(M.SCREEN_TRANSITION_OUT_STARTED, {
            screen = screen.id,
            next_screen = next_screen and next_screen.id,
        })
        active_transition_count = active_transition_count + 1
        change_context(screen)
        release_input(screen, next_screen)
        ...
        co = coroutine.create(function()
            ...
            if add_to_stack and top then
                ...
                if top and not top.popup then
                    same_screen = top and top.id == screen.id

                    if not same_screen then
                        show_out(top, screen, callbacks.track())
            ...
            if not same_screen then
                ...
                show_in_already_visible(screen, top, options and options.reload, add_to_stack, callbacks.track())
            end
