...
function RPass:tostring_char()
    return '.'
end
function RPass:add(s, name)
    s.pass[name] = self
end
function RPass:type()
    return 'pass'
...
function RPass:tostring(name)
    return fmt('PASS: %s%s%s', name or '(unknown)', msec(self.elapsed), self.msg and (': ' .. tostring(self.msg)) or '')
end

local RFail = {}
local failMT = {__index = RFail}
function RFail:tostring_char()
    return 'F'
end
function RFail:add(s, name)
    s.fail[name] = self
end
function RFail:type()
    return 'fail'
...
function RSkip:tostring_char()
    return 's'
end
function RSkip:add(s, name)
    s.skip[name] = self
end
function RSkip:type()
    return 'skip'
...
function RSkip:tostring(name)
    return fmt('SKIP: %s()%s', name or 'unknown', self.msg and (' - ' .. tostring(self.msg)) or '')
end

local RError = {}
local errorMT = {__index = RError}
function RError:tostring_char()
    return 'E'
end
function RError:add(s, name)
    s.err[name] = self
end
function RError:type()
    return 'error'
end
function RError:tostring(name)
    return self.msg or fmt("ERROR (in %s%s, couldn't get traceback)", msec(self.elapsed), name or '(unknown)')
end

local function Pass(t)
    return setmetatable(t or {}, passMT)
end
local function Fail(t)
    return setmetatable(t, failMT)
end
local function Skip(t)
    return setmetatable(t, skipMT)
end
local function Error(t)
    return setmetatable(t, errorMT)
end
...
function lunatest.assert_not_equal(exp, got, msg)
    wraptest(exp ~= got, msg, {reason = 'Expected something other than ' .. TS(exp)})
end
...
function lunatest.assert_not_len(len, val, msg)
    wraptest(#val ~= len, msg, {reason = fmt('Expected length other than %d', len)})
end
...
function lunatest.assert_not_match(pat, s, msg)
    wraptest(type(s) ~= 'string' or not s:match(pat), msg, {reason = fmt('Should not match pattern %s', pat)})
end
...
verbose_hooks = {
    ...
    begin_suite = function(s_env, tests)
        dot_ct = 0
        printf('-- Starting suite %q, %d test(s)', s_env.name, count(tests))
    end,
    end_suite = function(s_env)
        local ps, fs = count(s_env.pass), count(s_env.fail)
        local ss, es = count(s_env.skip), count(s_env.err)
        dot_ct = 0
        printf('    Finished suite %q, +%d -%d E%d s%d', s_env.name, ps, fs, es, ss)
    end,
    pre_test = false,
    post_test = function(name, res)
        printf('%s', res:tostring(name))
...
local function err_handler(name)
    return function(e)
        if type(e) == 'table' and e.type and ok_types[e.type()] then
            return e
        end
        local msg = fmt('\nERROR in %s():\n\t%s', name, tostring(e))
...
local function run_test(name, test, suite, hooks, setup, teardown)
    ...
    if is_func(hooks.post_test) then
        hooks.post_test(name, result)
    end
end
...
local function run_suite(hooks, opts, results, sname, tests)
    ...
    if not opts.suite_pat or sname:match(opts.suite_pat) then
        ...
        if run_suite and count(tests) > 0 then
            ...
            if hooks.begin_suite then
                hooks.begin_suite(res, tests)
            end
            res.tests = tests
...
local function parse_pattern(pattern)
    ...
    local function at_either_end()
        return #cs == 0 or #cs == len
    end
    local function slice(i)
        return string.sub(pattern, i, i)
    end
...
function lunatest.assert_random(opt, f, ...)
    ...
    for i = 1, opt.count do
        ...
        if #r.ss >= opt.max_skips or #r.fs >= opt.max_failures or #r.es >= opt.max_errors then
            break
        end
